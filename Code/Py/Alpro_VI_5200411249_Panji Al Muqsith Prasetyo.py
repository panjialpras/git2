...
def lihat_bookingan () :
    if (len(data_booking) == 0) :
        ...
        print("-" * 30)
        print("Nama", "Lama ", "Tanggal ", "Jam ", "Menit ", "Selesai", "Harga")
        print("-" * 30)

        for data in data_booking :
            print(data["Nama"], str(data["Lama sewa"]), str(data["Tanggal sewa"]), str(data["Jam Mulai"]), str(data["Menit Mulai"]), str(data["Jam_Selesai"]), 4000 * float (data["Jam Mulai"]))
